...
Assert-Condition ($solucoesCss -match '(?s)\.service-node-icon::before\s*\{.*?background:\s*(?:#ffffff|#fff|#061226)') 'icones precisam de uma camada opaca para bloquear as linhas do radar.'
Assert-Condition ($solucoesCss -match '(?s)\.service-node\.is-active \.service-node-icon\s*\{.*?background:\s*') 'o icone ativo precisa de destaque no radar.'
