...
Assert-Condition ($solucoesCss -match '--radar-sweep-core-opacity') 'solucoes.css precisa centralizar a intensidade do nucleo do sweep.'
Assert-Condition ($solucoesCss -match '--radar-sweep-halo-opacity') 'solucoes.css precisa centralizar a intensidade do halo do sweep.'
Assert-Condition ($solucoesCss -match '--radar-sweep-tail-opacity') 'solucoes.css precisa centralizar a intensidade da cauda do sweep.'
Assert-Condition ($solucoesCss -match 'filter:\s*blur\(3px\)') 'o sweep precisa ter uma camada de halo difuso.'
...
Assert-Condition ($solucoesCss -match '(?s)\.service-node-icon::before\s*\{.*?background:\s*#061226') 'icones precisam de uma camada opaca para bloquear as linhas do radar.'
Assert-Condition ($solucoesCss -match '(?s)\.service-node\.is-active \.service-node-icon\s*\{.*?background:\s*rgba\(0, 156, 255, 0\.28\)') 'o icone ativo precisa preservar o azul translúcido original.'
