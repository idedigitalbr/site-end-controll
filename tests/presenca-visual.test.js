...
test('uses the standard navy across the national presence section accents', () => {
  ...
  assert.match(home, /presenca-nacional\.css\?v=33\.0/);
});

test('matches the Sobre a EndControl badge and headline typography', () => {
  assert.match(
    styles,
    /\.presenca-eyebrow\s*\{[\s\S]*?font-size:\s*0\.82rem\s*;[\s\S]*?font-weight:\s*700\s*;[\s\S]*?color:\s*#ffffff\s*;[\s\S]*?background:\s*#00215D\s*;[\s\S]*?border-radius:\s*999px\s*;[\s\S]*?padding:\s*8px\s+16px\s*;/i,
  );
  assert.match(
    styles,
    /\.presenca-heading\s*\{[\s\S]*?font-size:\s*clamp\(2rem,\s*3\.2vw,\s*2\.85rem\)\s*;[\s\S]*?font-weight:\s*800\s*;[\s\S]*?line-height:\s*1\.15\s*;[\s\S]*?color:\s*#07152F\s*;/i,
  );
});
