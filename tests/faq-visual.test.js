...
const faqGridRule = styles.match(/\.faq-v2-grid\s*\{[^}]*\}/i)?.[0] || '';
...
test('keeps inactive FAQ cards at their own height when one answer opens', () => {
  assert.match(
    faqGridRule,
    /align-items:\s*(?:start|flex-start)\s*;/i,
    'the FAQ grid must not stretch the neighboring card to the active answer height'
  );
});
