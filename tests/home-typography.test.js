...
test('reapplies the shared badge, headline, italic highlight and description pattern across Home sections', () => {
  ...
  assert.match(heroStyles, /\.hero-badge-pill\s*\{(?=[^}]*font-size:\s*var\(--font-size-eyebrow,\s*12px\))(?=[^}]*letter-spacing:\s*0\.14em)(?=[^}]*padding:\s*8px 14px)[^}]*\}/i);
  ...
  assert.match(solutionStyles, /\.solucoes-badge-text\s*\{(?=[^}]*font-size:\s*var\(--font-size-eyebrow,\s*12px\))(?=[^}]*letter-spacing:\s*0\.14em)[^}]*\}/i);
